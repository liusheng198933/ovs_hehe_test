    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    RULE_COLLECTION_FOR_EACH (rule, &rules) {
        ...
        if (rule->cr.rtmp > 0) {
            fs.match.flow.vlan_tci = htons(rule->cr.rtmp+4096);
            fs.match.wc.masks.vlan_tci = 65311;}    // change
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    //sprintf(filepath, "/home/shengliu/Workspace/mininet/haha/API/time/debug_%d.txt", rand() % 200);
    //sprintf(filepath, "/home/shengliu/Workspace/mininet/haha/API/time/debug.txt");

    //struct timeval tval_cur;

    switch (ofm.command) {
    case OFPFC_ADD:
      // if (vlan_tci_to_vid(fm->match.flow.vlan_tci) > 0)
      // {
      //   filep = fopen(filepath, "aw+");
      //   //fprintf(filep, "field: %i, wc: %i, rtmp: %i\n", flowcp->vlan_tci, flowwc->vlan_tci, rule->rtmp);
      //   fprintf(filep, "add prt: %i outport: %i src: %i dst: %i rtmp: %i\n", fm->priority, fm->out_port, fm->match.flow.nw_src, fm->match.flow.nw_dst, vlan_tci_to_vid(fm->match.flow.vlan_tci));
      //   fclose(filep);
      // }
        break;
    ...
    case OFPFC_DELETE_STRICT:
        ...
        //   fprintf(filep, "del prt: %i outport: %i src: %i dst: %i rtmp: %i time %ld %ld\n", fm->priority, fm->out_port, fm->match.flow.nw_src, fm->match.flow.nw_dst, vlan_tci_to_vid(fm->match.flow.vlan_tci), (long int)tval_cur.tv_sec, (long int)tval_cur.tv_usec);
        //   fclose(filep);
        // }
        break;
    }
...
handle_bundle_control(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    case OFPBCT_COMMIT_REQUEST:
        ...
        //struct timeval tval_cur;
        //gettimeofday(&tval_cur, NULL);
        //fprintf(filep, "bdid %i rules commit time %ld %ld\n", bctrl.bundle_id, (long int)tval_cur.tv_sec, (long int)tval_cur.tv_usec);
        //fclose(filep);
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    if (!error) {
        ...
        //filep = fopen(filepath, "aw+");
                                           ...
                                           //fprintf(filep, "field: %i, wc: %i, rtmp: %i\n", flowcp->vlan_tci, flowwc->vlan_tci, rule->rtmp);
        //fprintf(filep, "bundle add bundle_id %i cmd %i\n", badd.bundle_id, bmsg->ofm.command);
        //fclose(filep);
